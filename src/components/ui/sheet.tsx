...
>(({ className, ...props }, ref) => (
    <SheetPrimitive.Overlay
        className={cn(
            'desktop:rounded-[10px] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-[#f6f6f3]/60 dark:bg-black/60',
            className
        )}
        {...props}
        ref={ref}
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
    ({ side = 'right', stack = false, className, children, title, ...props }, ref) => (
        <SheetPortal>
            <SheetOverlay />
